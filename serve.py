...
class Transcriber:
    ...
    def transcribe(self, uid, transcript, audio, async_mode, **kwargs):
        ...
        if gentle.resample(os.path.join(outdir, "upload"), wavfile) != 0:
            status["status"] = "ERROR"
            status["error"] = (
                "Encoding failed. Make sure that you've uploaded a valid media file."
            )
        ...
        if len(transcript.strip()) > 0:
            ...
            status["status"] = "ERROR"
            status["error"] = (
                "No transcript provided and no language model for full transcription"
            )
            return
...
class TranscriptionsController(Resource):
    ...
    def render_POST(self, req):
        ...
        # Copy over the HTML
        shutil.copy(
            get_resource("www/view_alignment.html"), os.path.join(outdir, "index.html")
        )
...
class LazyZipper(Insist):
    ...
    def serialize_computation(self, outpath):
        shutil.make_archive(
            ".".join(
                outpath.split(".")[:-1]
            ),  # We need to strip the ".zip" from the end
...
def serve(
    ...
    trans = Transcriber(
        data_dir, nthreads=nthreads, ntranscriptionthreads=ntranscriptionthreads
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--host", default="0.0.0.0", help="host to run http server on")
    parser.add_argument(
        "--port", default=8765, type=int, help="port number to run http server on"
    )
    parser.add_argument(
        "--nthreads",
